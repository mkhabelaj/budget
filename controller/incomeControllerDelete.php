...
if(isset($_POST)){
    ...
    printItemBreakMany($time_line_id,$budget_id);
    $sql="SELECT COUNT(*) AS total FROM income WHERE time_line_id=".$time_line_id." AND budget_instance_ID=".$budget_id;
    if ( dataBaseManipulation($sql,con(),"rows","selecting from income",true)["total"] > 1):
        $sql="DELETE  FROM income WHERE income_id=".$income_id;
        dataBaseManipulation($sql,con(),"conn","deleting from income",true);
    else:
        printItemBreak("each budget must have at least on income");
    endif;
